...
- *Фикс:* `parseAWGNumeric` (`node_parser_wireguard.go`): одиночное число — как раньше; `lo-hi` (оба uint32) — **схлопывается в начало диапазона** (любое значение внутри валидно для сервера; начало даёт детерминизм parse → share → parse). Перевёрнутый диапазон терпится (берётся меньшая граница). Невалидное значение — прежняя политика: скип поля + debug-log, узел живёт.
- *Ограничение:* рандомизация per-handshake на стороне ядра потребовала бы поддержки диапазонов в shape sing-box-lx (`h1`–`h4` — uint32); пока ядро не умеет — фиксированное значение из диапазона корректно.
- *Тесты:* `awg_range_test.go` — диапазоны в URI (прямой/перевёрнутый/мусор) и полный AWG2-`.conf` с диапазонами и пустыми `I2–I5`.
