...
3. **Валидация (build-time, в генераторе)** перед эмиссией, зная полный набор тегов:
   - **висячий detour** (тег не существует среди итоговых outbound'ов) → WarnLog + убрать поле (узел dial'ится напрямую, конфиг не падает);
   - **самоссылка** (detour-тег = собственный тег узла) → убрать + WarnLog;
   - **цикл** (`A.detour=B`, `B.detour=A`, и длиннее) → детектировать, разорвать на одном звене + WarnLog.
